...
from config.toy_oscillator_adv_config import ToyOscillatorAdvConfig
from model.toy_oscillator_adv_pinn import ToyOscillatorAdvPINN
from trainer.toy_oscillator_adv_trainer import train_toy_oscillator_adv, grad
...
def main():
    ...
    x0_pred, _, _, _, _ = model(t0, g0, w0)
    ...
    # residual of original ODE for plot
    t_req = t.clone().requires_grad_(True)
    x_pred, _, _, _, _ = model(t_req, gamma, omega)
    ...
    plot_prediction(
        ...
        x_true=x_true,
        x_pred=x_hat,
        title=f"Toy PINN (Adv FULL): x''+2γx'+ω²x=0 | γ={gamma_test}, ω={omega_test}",
        out_path="outputs/toy_oscillator_adv_full_prediction.png"
    )
    plot_residual(
        t=t_np,
        res=res_x,
        title="Toy PINN (Adv FULL): pointwise residual of original ODE",
        out_path="outputs/toy_oscillator_adv_full_residual.png"
    )

    rel_l2 = np.linalg.norm(x_hat - x_true) / (np.linalg.norm(x_true) + 1e-15)
    print(f"relative L2 error on test params: {rel_l2:.3e}")
    print("saved:")
    print("  outputs/toy_oscillator_adv_full_prediction.png")
    print("  outputs/toy_oscillator_adv_full_residual.png")
